...
def test_get_tag_not_found(client, sample_tags):
...
def test_update_tag(client, sample_tags, test_session):
    update_data = {
        "tag": "hilarious",
...
def test_update_tag_not_found(client, sample_tags):
    response = client.put("/tags/999", json={"tag": "test"})
    
    assert response.status_code == 404
    data = response.get_json()
    assert "error" in data

def test_delete_tag(client, sample_tags, test_session):
...
def test_delete_tag_not_found(client, sample_tags):
